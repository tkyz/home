...
if [[ -f "${HOME_YML}" ]]; then
  ...
  if true; then
    ...
    mkdir -p "${HOME_DIR}/local/sbin/"

    root_pub="$(echo "${yml}" | yq -r '.root.pub' | envsubst)"
    root_crt="$(echo "${yml}" | yq -r '.root.crt' | envsubst)"
    node_key="$(echo "${yml}" | yq -r '.node.key' | envsubst)"
    node_pub="$(echo "${yml}" | yq -r '.node.pub' | envsubst)"
    node_csr="$(echo "${yml}" | yq -r '.node.csr' | envsubst)"
    node_crt="$(echo "${yml}" | yq -r '.node.crt' | envsubst)"
...
#   openssl rsa -in "${node_key}" -pubout -out "${node_pub}.der" -outform der 2> /dev/null
  ...
  if true; then
    ...
    tmp_file="$(mktemp)"
    {
      echo "$(cat "${node_pub}" | ssh-keygen -f /dev/stdin -i -m pkcs8) node-pub ${USER}@$(hostname -f)"
      echo "$(cat "${root_pub}" | ssh-keygen -f /dev/stdin -i -m pkcs8) root-pub ${USER}@$(hostname -f)"
    ...
    # TODO: 重複登録される
    if false; then
      cat "${HOME}/.ssh/authorized_keys" #| ssh "${USER}@keys.tkyz.jp" 'mkdir -p ~/.ssh/; cat >> ~/.ssh/authorized_keys'
    fi
 ...
 # origin_ssh='ssh://git.home/home.git'
   ...
   elif ! is_docker && ! is_wsl && is_tcp git.home 22; then # TODO: 鍵がない判定
